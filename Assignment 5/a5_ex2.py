...
import torch, tqdm, time, matplotlib, numpy
def training_loop(
    ...
    eval_losses = []
    #training_loss_minimum = min(training_loss_averages)
    ...
    for epoch in range(num_epochs):
        ...
        if len(training_loss_averages) > 0:
            training_loss_minimum = min(training_loss_averages)
        if epoch > 7 and training_loss_minimum not in training_loss_averages[epoch-7:epoch]:
            break
...
def plot_losses(train_losses: list, eval_losses: list):
    x_points = numpy.array([epoch for epoch in range(len(train_losses))])
    y_points_train = numpy.array([train_loss.detach().numpy() for train_loss in train_losses])
    y_points_eval = numpy.array([eval_loss.detach().numpy() for eval_loss in eval_losses])
    matplotlib.pyplot.plot(x_points, y_points_train, label='Train Loss')
    matplotlib.pyplot.plot(x_points, y_points_eval, label='Eval Loss')
    matplotlib.pyplot.title("Network Loss")
    matplotlib.pyplot.xlabel("Epochs")
    matplotlib.pyplot.ylabel("Loss")
    matplotlib.pyplot.legend(loc="upper right")
    matplotlib.pyplot.savefig("epoch_loss.pdf", format="pdf")
...
if __name__ == "__main__":
    ...
    train_losses, eval_losses = training_loop(network, train_data, eval_data, 
                                              num_epochs=100, batch_size=16, learning_rate=1e-3)
